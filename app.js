...
async function saveAsDefaultPriceList() {
  if (!confirm('Czy na pewno chcesz ustawić aktualny stan cennika jako swój domyślny punkt przywracania (do resetu)?')) return;
  try {
    await fetch('/api/products/default', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: products }) });
    showToast('Zapisano aktualny cennik jako domyślny punkt do resetu', 'success');
  } catch (err) {
    console.error('saveAsDefaultPriceList error:', err);
    showToast('Błąd zapisu cennika domyślnego', 'error');
  }
}

function downloadPricelistFile() {
  const fileContent = `/* ===== DEFAULT PRODUCT DATA ===== */\nconst DEFAULT_PRODUCTS = ${JSON.stringify(products, null, 4)};\n\nconst CATEGORIES = ${JSON.stringify(CATEGORIES, null, 4)};\n\n/* ===== GASKET AUTO-ADD MAP ===== */\nconst GASKET_MAP = ${JSON.stringify(GASKET_MAP, null, 4)};\n`;
  const blob = new Blob([fileContent], { type: 'application/javascript' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'pricelist.js';
  a.click();
