...
- **Konwersja krag/krag_ot**: wpisanie ilości w kolumnie `krag`/`krag_ot` usuwa **wszystkie** kręgi obu typów o danym dn+height (bez sumowania), a finalny typ ustala `enforceOtRings()` wg geometrii otworów. Test regresyjny: `tests/studnie/excelDrilledRings.test.ts`.
...
| 20  | **Duplikacja kręgów krag/krag_ot (Excel)**             | W bloku konwersji `excelOnCompChange` sumowano `totalQty = totalExistingQty + newQty` zamiast zastąpienia, a filtr usuwał tylko wpisany typ — bratni typ (krag/krag_ot) o tym samym dn+height zostawał w tabeli.                                                                                                                    | Usuwać wszystkie `krag` i `krag_ot` o danym dn+height (wpisany typ **i** bratni typ), bez sumowania; wstawiać element wpisanego typu z ilością = wpisana (`newQty`), a finalny typ (krag vs krag_ot) ustala `enforceOtRings()` wg geometrii otworów. Test regresyjny: `tests/studnie/excelDrilledRings.test.ts`. |
