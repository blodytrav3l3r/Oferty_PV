...
| 25  | **Brak esbuild przy Vite 8 (build frontendu)**                                               | Vite 8 ma `esbuild` jako opcjonalną zależność (optional peer dep) — bez niej build z `minify: 'esbuild'` w `vite.config.js` kończy się błędem.                                                                                                                                                                                      | `npm install --save-dev esbuild` (wymagane dla `minify: 'esbuild'`); przy aktualizacji Vite sprawdź, czy wszystkie pakiety z `optionalDependencies` są faktycznie zainstalowane.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 29  | **Brak snapshotów undo dla części mutacji + przepełnienie stacku przy wklejaniu (Excel)**    | `_excelSaveUndoSnapshot()` wołano tylko w `excelOnCompChange` i wklejaniu — zmiany rzędnej, przejść, typu przejścia, kinety i psiej budy nie były cofalne (Ctrl+Z bez efektu). Przy wklejaniu wielokomórkowym snapshot per komórka przepełniał stack (limit `_EXCEL_UNDO_LIMIT = 20`) i Ctrl+Z nie cofał wklejenia.                 | Snapshot na początku **każdego** mutującego handlera: `excelOnRzednaChange`, `excelOnWlazChange`, `excelOnPrzejscieChange`, `excelOnPrzejscieTypeChange`, `excelOnKinetaChange`, `excelOnPsiaBudaChange`, `excelOnReductionSelectChange`. Wklejanie: **jeden** snapshot w `_excelHandlePaste` + flaga `_excelPasteInProgress` — handler pomija snapshot gdy flaga ustawiona (`if (!_excelPasteInProgress) _excelSaveUndoSnapshot()`). Batch (>100 wierszy) resetuje flagę w `doneCallback`, nie w `finally` — inaczej guard wygasa przed pierwszym tickiem rAF. **Testy vm nie ładują `excelState.js`** → używaj `typeof _excelPasteInProgress === 'undefined' |     | !_excelPasteInProgress`(inaczej`ReferenceError`w`excelDrilledRings.test.ts`).                                                                                                                                                                     |
