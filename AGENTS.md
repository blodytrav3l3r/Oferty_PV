...
- **Skróty klawiszowe (F1/F3)**: **Ctrl+Enter** wypełnia zaznaczony zakres (`_excelSelectedCells` + `_excelSelectedCols`) wartością z aktywnej komórki przez `_excelHandleFillDown` (excelCopyPaste.js) — jeden snapshot undo, guard `_excelPasteInProgress`, pomija nazwę (colIdx 3), wiersze ukryte filtrem i studnie zablokowane (PZ); źródło NIE może być checkboxem ani kolumną strukturalną (colIdx ≤ 3). **Ctrl+D** bez zaznaczenia komórek duplikuje studnię (`excelDuplicateWell`), z zaznaczeniem = istniejący fill-down; gałęzie wykluczają się po `_excelSelectedCells.length`.
- **Trwałość szerokości kolumn (F2)**: `_excelColWidths` (klucz `_excelActiveTab + '-' + colIdx`) zapisywane do localStorage przez `_excelSaveColWidths()` na końcu `onUp` resize; ładowane w `openExcelTableModal`; try/catch + walidacja typu, błędny JSON → `{}`. Szerokości NIE wchodzą do `_excelResetLayoutDependentState()`.
- **Tła błędów konfiguracji (F4)**: `_excelGetRowStatus(well)` (excelTableBody.js) zwraca tint ERROR/WARNING (`rgba(var(--danger-rgb), 0.12)` / `rgba(var(--warn-rgb), 0.1)`) z priorytetem ERROR > WARNING > duplikat > aktywny > base; używany w `_excelRenderTbody` i `_excelRefreshDupColors`; po zmianie tła wiersza aktualizuj komórki sticky (`td:nth-child(-n+7)` przez `_excelStickyCellBg`). Status z głównego panelu dostaje się przez snapshot pollingowy (configStatus). **Nie używać klas `.well-row-error/.well-row-warning`** z oferty (`!important`).
- **Wyszukiwarka (F5)**: `#excel-search-input` szerokość 220px + przycisk `#excel-search-clear` (✕, `onclick="excelClearSearch()"`) — widoczny tylko przy aktywnym filtrze; `excelClearSearch()` czyści filtr, przywraca wiersze i focus.
