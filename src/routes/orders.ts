...
router.put('/', requireAuth as any, async (req, res) => {
  ...
  try {
    ...
    for (const o of incoming) {
      ...
      if (old) {
        logAudit('order', docId, authReq.user?.id || '', 'update', newData, parseJsonField(old.data));
      } else {
...
router.put('/production', requireAuth as any, async (req, res) => {
  ...
  try {
    ...
    for (const o of incoming) {
      ...
      const { id, type, userId, orderId, wellId, elementIndex, createdAt, updatedAt, ...rest } = o;
      const dataStr = JSON.stringify(rest);
      ...
      if (old) {
        logAudit('production_order', docId, authReq.user?.id || '', 'update', rest, parseJsonField(old.data));
      } else {
...
router.delete('/production/:id', requireAuth as any, async (req, res) => {
  ...
  try {
    ...
    if (oldData.status === 'accepted') {
      return res.status(403).json({ error: 'Nie można usunąć zatwierdzonego zlecenia. Najpierw je cofnij.' });
    }
...
router.patch('/:id', requireAuth as any, async (req, res) => {
  ...
  try {
    ...
    const isProParent = o && authReq.user?.role === 'pro' && (authReq.user?.subUsers || []).includes(o.userId);
