 ...
 */
router.post('/ai/events/bulk', requireAuth, WRITE_LIMITER, async (req, res) => {
    const authReq = req as AuthenticatedRequest;
    const userId = authReq.user?.id;
    ...
    try {
        const result = await telemetryService.recordEventsBulk(data.events, userId);
        return res.json(result);
    } catch (e) {
        const message = e instanceof Error ? e.message : String(e);
 ...
 * (pasywne uczenie).
 */
router.post('/ai/acceptance', requireAuth, WRITE_LIMITER, async (req, res) => {
 ...
 */
router.get('/ai/list', requireAuth, READ_LIMITER, async (req, res) => {
    const authReq = req as AuthenticatedRequest;
 ...
 */
router.get('/ai/history/:wellId', requireAuth, READ_LIMITER, async (req, res) => {
    const authReq = req as AuthenticatedRequest;
    if (authReq.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Brak uprawnień' });
    }
 ...
 */
router.get('/ai/transitions/:configId', requireAuth, READ_LIMITER, async (req, res) => {
    const authReq = req as AuthenticatedRequest;
    if (authReq.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Brak uprawnień' });
    }
 ...
 */
router.get('/ai/events/:wellId', requireAuth, READ_LIMITER, async (req, res) => {
    const authReq = req as AuthenticatedRequest;
    if (authReq.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Brak uprawnień' });
    }
 ...
 */
router.get('/ai/versions', requireAuth, READ_LIMITER, async (req, res) => {
    const authReq = req as AuthenticatedRequest;
    if (authReq.user?.role !== 'admin') {
        return res.status(403).json({ error: 'Brak uprawnień' });
    }
