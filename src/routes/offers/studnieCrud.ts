...
router.get('/studnie', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const mapped = offers.map((offer) => {
            ...
            if (offer.data) {
                try { studnieSpread = JSON.parse(offer.data); } catch { studnieSpread = {}; }
            }
...
router.get('/studnie/:id', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (authReq.user?.role !== 'admin' && offer.userId !== authReq.user?.id) {
            ...
            try { studnieDetailHistory = JSON.parse(offer.history || '[]'); } catch { studnieDetailHistory = []; }
    ...
    try {
        ...
        for (const o of incoming) {
            ...
            if (old) {
                ...
                try { snapshotData = JSON.parse(old.data || '{}'); } catch { snapshotData = {}; }
                const snapshot = {
                ...
                try { auditData = JSON.parse(old.data || '{}'); } catch { auditData = {}; }
                logAudit(
                    'studnia_oferta',
                    docId,
                    authReq.user?.id || '',
