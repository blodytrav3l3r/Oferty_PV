    ...
    for (const o of incoming) {
      ...
      if (old) {
        ...
        logAudit(
          ...
          authReq.user?.id || '',
          'update',
          { state: o.status === 'active' ? 'final' : 'draft', transportCost: o.transportCost, items: o.items },
          snapshot
        );
      } else {
        logAudit(
...
router.post('/studnie', requireAuth as any, async (req, res) => {
  ...
  try {
    ...
    console.log(`[POST] Zapisano ${results.length} ofert studnie przez ${authReq.user?.username}`);
    res.json({ ok: true, results });
  } catch (e: any) {
...
router.delete('/:id', requireAuth as any, async (req, res) => {
  ...
  try {
    ...
    if (authReq.user?.role !== 'admin' && offer.userId !== authReq.user?.id) {
      return res
        .status(403)
        .json({ error: 'Brak uprawnien do usuniecia tej oferty' });
    }
