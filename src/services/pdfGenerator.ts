...
export async function generateOfferStudniePDF(offerId: string): Promise<Buffer> {
  ...
  const html = await generateStudnieHTML({
    ...
    clientPhone: offerData.clientContact || client?.contact || client?.phone || offerData.clientPhone || '',
...
export async function lookupOfferUsers(
  ...
  if (guardianId) {
    try {
      ...
      if (u) guardianUser = { name: formatUserName(u), email: u.email || '', phone: u.phone || '' };
    } catch (e) { console.warn('lookupOfferUsers: guardian lookup failed', e); }
  }
  ...
  if (authorId && authorId !== guardianId) {
    try {
      ...
      if (u) authorUser = { name: formatUserName(u), email: u.email || '', phone: u.phone || '' };
    } catch (e) { console.warn('lookupOfferUsers: author lookup failed', e); }
...
export async function generateStudnieHTML(data: StudnieOfferData): Promise<string> {
  ...
  for (const dn of dnOrder) {
    ...
    const rows = dnItems.map((item, idx) => {
  ...
  for (const dn of dnOrder) {
    if (itemsByDN[dn]) {
      ...
      const total = itemsByDN[dn].reduce((sum: number, item: any) => sum + (item.price || 0), 0);
  ...
  ` : '';
  const payTerms = data.paymentTerms || 'Do uzgodnienia lub według indywidualnych warunków handlowych.';
