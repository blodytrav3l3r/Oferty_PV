import {
  ...
  WidthType, AlignmentType, Header, Footer, PageNumber, ImageRun,
...
type BorderDef = { style: (typeof BorderStyle)[keyof typeof BorderStyle]; size: number; color: string };
...
const NO_BORDERS: CellBorders = { top: BORDER_NONE, bottom: BORDER_NONE, left: BORDER_NONE, right: BORDER_NONE };
const CELL_BORDERS: CellBorders = { top: BORDER_THIN, bottom: BORDER_THIN, left: BORDER_THIN, right: BORDER_THIN };
...
function textCell(
  text: string,
  opts: {
    bold?: boolean; size?: number; alignment?: (typeof AlignmentType)[keyof typeof AlignmentType];
    fill?: string; color?: string; width?: number; columnSpan?: number;
    borders?: CellBorders;
  } = {}
): TableCell {
...
function buildImageHeader(): Header {
  ...
  if (!imgData) {
    return new Header({
      children: [new Paragraph({
        children: [new TextRun({ text: 'P.V. PREFABET KLUCZBORK S.A.', bold: true, size: 28, font: FONT, color: 'CC0000' })],
...
function buildImageFooter(): Footer {
  ...
  children.push(new Paragraph({
    children: [
      ...
      new TextRun({ children: [PageNumber.CURRENT], size: 14, color: COLOR_GRAY_HEADER, font: FONT }),
      new TextRun({ text: ' z ', size: 14, color: COLOR_GRAY_HEADER, font: FONT }),
      new TextRun({ children: [PageNumber.TOTAL_PAGES], size: 14, color: COLOR_GRAY_HEADER, font: FONT }),
    ],
    alignment: AlignmentType.CENTER,
...
export async function generateOfferStudnieDOCX(offerId: string): Promise<Buffer> {
  ...
  const client = offer.clientId ? await prisma.clients_rel.findUnique({ where: { id: offer.clientId } }) : null;
...
function buildStudnieDocument(
  ...
  const paymentTerms = offerData.paymentTerms || 'Do uzgodnienia lub według indywidualnych warunków handlowych.';

  const children: (Paragraph | Table)[] = [];

  // 1. Tytuł: 16pt, bold, centered, margin 8px top 4px bottom
  children.push(new Paragraph({
    children: [new TextRun({ text: `OFERTA HANDLOWA ${offerNumber}`, bold: true, size: SZ_TITLE, font: FONT, color: '000000' })],
    alignment: AlignmentType.CENTER,
  ...
  children.push(buildClientInvestTable(clientName, clientNip, clientAddress, clientContact, investName, investAddress, investContractor));
    ...
    sections: [{
      properties: {
        page: {
          ...
          margin: { top: 60, bottom: 280, right: 570, left: 570, header: 60, footer: 280 },
        },
      },
...
function buildClientInvestTable(
  ...
  if (address) { clientRuns.push(new TextRun({ break: 1 } as any)); clientRuns.push(new TextRun({ text: address, size: SZ_INFO_BOX, font: FONT, color: '000000' })); }
  if (nip) { clientRuns.push(new TextRun({ break: 1 } as any)); clientRuns.push(new TextRun({ text: `NIP: ${nip}`, size: SZ_INFO_BOX, font: FONT, color: '000000' })); }
  if (contact) { clientRuns.push(new TextRun({ break: 1 } as any)); clientRuns.push(new TextRun({ text: `Kontakt: ${contact}`, size: SZ_INFO_BOX, font: FONT, color: '000000' })); }

  // Inwestycja
  const investRuns: TextRun[] = [];
  if (investName) {
    investRuns.push(new TextRun({ text: 'Budowa: ', bold: true, size: SZ_INFO_BOX, font: FONT, color: '000000' }));
    investRuns.push(new TextRun({ text: investName, size: SZ_INFO_BOX, font: FONT, color: '000000' }));
  } else {
    investRuns.push(new TextRun({ text: '\u2014', size: SZ_INFO_BOX, font: FONT, color: '000000' }));
  }
  if (investAddress) { 
    investRuns.push(new TextRun({ break: 1 } as any)); 
    investRuns.push(new TextRun({ text: 'Adres: ', bold: true, size: SZ_INFO_BOX, font: FONT, color: '000000' }));
    investRuns.push(new TextRun({ text: investAddress, size: SZ_INFO_BOX, font: FONT, color: '000000' })); 
  }
  if (investContractor) { 
    investRuns.push(new TextRun({ break: 1 } as any)); 
    investRuns.push(new TextRun({ text: 'Wykonawca: ', bold: true, size: SZ_INFO_BOX, font: FONT, color: '000000' }));
    investRuns.push(new TextRun({ text: investContractor, size: SZ_INFO_BOX, font: FONT, color: '000000' })); 
  }
...
interface DnSummary { label: string; count: number; totalPrice: number }
...
function buildWellTables(wells: any[]): {
  ...
  const thStyle = {
    bold: true, size: SZ_TABLE_HEADER, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE,
    alignment: AlignmentType.CENTER,
  };
  ...
  for (const dn of dnOrder) {
    ...
    paragraphs.push(new Paragraph({
      children: [new TextRun({ text: dnLabel, bold: true, size: SZ_DN_HEADER, font: FONT, color: COLOR_GRAY_HEADER })],
    ...
    rows.push(new TableRow({
      children: [
        ...
        textCell(`Razem ${dnLabel} (${dnItems.length} szt.):`, {
          bold: true, size: SZ_TABLE_BODY, alignment: AlignmentType.RIGHT, fill: COLOR_BG_SUMMARY,
        }),
        textCell(`${fmtCurrency(dnTotal)} PLN`, {
          bold: true, size: SZ_TABLE_BODY, alignment: AlignmentType.CENTER, fill: COLOR_BG_SUMMARY,
        }),
      ],
    }));
...
function buildSummarySection(summaries: DnSummary[], grandTotal: number): (Paragraph | Table)[] {
  ...
  result.push(new Paragraph({
    children: [new TextRun({ text: 'Podsumowanie oferty', bold: true, size: SZ_DN_HEADER, font: FONT, color: COLOR_GRAY_HEADER })],
    spacing: { before: 160, after: 60 },
    border: { bottom: { style: BorderStyle.SINGLE, size: 2, color: COLOR_GRAY_HEADER } },
  }));

  // Wiersze: 10pt, padding 4px 8px, border 0.5pt #ccc
  const rows: TableRow[] = summaries.map(s => new TableRow({
    children: [
...
function buildContactSection(
  ...
  if (!guardianUser && !authorUser) {
    result.push(new Paragraph({
      children: [new TextRun({ text: 'W razie pytań prosimy o kontakt z opiekunem oferty.', size: SZ_TABLE_BODY, font: FONT })],
      spacing: { before: 40 },
    }));
    return result;
  }
...
function buildStaticTerms(): (Paragraph | Table)[] {
  ...
  result.push(new Paragraph({
    children: [new TextRun({ text: 'Informacje dodatkowe i ogólne warunki:', bold: true, size: SZ_TERMS_HEADER, font: FONT, color: COLOR_GRAY_HEADER })],
  ...
  const addP = (text: string, opts?: { bold?: boolean; marginTop?: number }) => {
    result.push(new Paragraph({
      children: [new TextRun({ text, size: SZ_TERMS, font: FONT, color: '333333', bold: opts?.bold })],
  ...
  result.push(new Paragraph({
    children: [new TextRun({
      ...
      bold: true, size: SZ_THANKS, font: FONT, color: '333333',
    })],
...
async function generateRuryDOCX(data: RuryOfferData): Promise<Buffer> {
  const totalNet = data.items.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity || 1)), 0);
  ...
  const tableRows = [
    ...
    ...data.items.map((item, idx) => new TableRow({
      children: [
        ...
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: item.productId || 'Produkt', size: 18 })] })] }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: String(item.quantity || 1), size: 18 })], alignment: AlignmentType.CENTER })] }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: item.discount ? item.discount + '%' : '-', size: 18 })], alignment: AlignmentType.RIGHT })] }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: formatCurrency(item.price || 0), size: 18 })], alignment: AlignmentType.RIGHT })] }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: formatCurrency((item.price || 0) * (item.quantity || 1)), size: 18 })], alignment: AlignmentType.RIGHT })] }),
      ],
    })),
  ];
    ...
    sections: [{
      ...
      headers: {
        default: new Header({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'WITROS', bold: true, size: 32, color: '333333' })],
              alignment: AlignmentType.CENTER,
              spacing: { after: 100 },
            }),
            new Paragraph({
              children: [new TextRun({ text: 'Oferta handlowa - Rury betonowe', size: 20, color: '666666' })],
              alignment: AlignmentType.CENTER,
              spacing: { after: 200 },
            }),
          ],
      ...
      footers: {
        default: new Footer({
          children: [
            new Paragraph({
              children: [new TextRun({ text: 'Oferta ważna przez 30 dni od daty wystawienia | WITROS - Generator Ofert', size: 16, color: '666666' })],
              alignment: AlignmentType.CENTER,
            }),
            new Paragraph({
              children: [
      ...
      children: [
        new Paragraph({
          children: [new TextRun({ text: 'Dane oferty', bold: true, size: 20, color: '666666' })],
        ...
        new Paragraph({
          children: [new TextRun({ text: 'Dane klienta', bold: true, size: 20, color: '666666' })],
          spacing: { before: 200, after: 100 },
        }),
        new Paragraph({
        ...
        data.clientNip ? new Paragraph({
          children: [new TextRun({ text: 'NIP: ', size: 22 }), new TextRun({ text: data.clientNip, size: 22 })],
          spacing: { after: 50 },
        }) : new Paragraph({ children: [] }),
        data.clientAddress ? new Paragraph({
          children: [new TextRun({ text: data.clientAddress, size: 22 })],
          spacing: { after: 200 },
        }) : new Paragraph({ children: [] }),
        new Paragraph({
          children: [new TextRun({ text: 'Pozycje ofertowe', bold: true, size: 20, color: '666666' })],
          spacing: { before: 200, after: 100 },
        }),
        new Table({ rows: tableRows, width: { size: 100, type: WidthType.PERCENTAGE } }),
        new Paragraph({
          children: [new TextRun({ text: 'Podsumowanie', bold: true, size: 20, color: '666666' })],
        ...
        data.transportCost > 0 ? new Paragraph({
          children: [
            new TextRun({ text: 'Koszt transportu: ', size: 22 }),
            new TextRun({ text: formatCurrency(data.transportCost), size: 22 }),
          ],
          spacing: { after: 50 },
        }) : new Paragraph({ children: [] }),
        new Paragraph({
          children: [
            ...
            new TextRun({ text: formatCurrency(totalWithTransport), bold: true, size: 26, color: '333333' }),
          ],
          spacing: { after: 200 },
        }),
      ],
