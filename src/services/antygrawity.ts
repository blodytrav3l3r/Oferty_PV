 ...
 */
export async function selectManholeComponents(
  ...
  try {
    ...
    if (!dennica) {
      result.errors.push(
        `Brak dostępnej dennicy dla DN${targetDn} w magazynie ${magazyn}`
      );
    ...
    if (!ending) {
      result.errors.push(
        `Brak zakończenia ${zakonczenieType} dla DN${reductionDn}`
      );
      result.success = false;
      return result;
    }
    ...
    if (needsReduction) {
      ...
      if (!reductionPlate) {
        result.errors.push(
          `Brak płyty redukcyjnej DN${targetDn}/DN${reductionDn}`
        );
        result.success = false;
        return result;
      }
    ...
    if (przejscia.length > 0) {
      ...
      if (passageResult.requiresOtRing) {
        const otRing = await selectKragOt(
          currentDn,
          magazynField,
          formaStdField
        );
...
async function selectPlytaRedukcyjna(
  ...
  for (const plate of plates) {
    if (
      plate.name?.includes(`DN${toDn}`) ||
      plate.id?.includes(`${toDn}`)
    ) {
      return plate;
    }
  }
...
async function placePrzejscia(
  ...
  for (const przejscie of przejscia) {
    ...
    for (const component of components) {
      ...
      if (
        height_from_bottom >= cumulativeHeight &&
        height_from_bottom <= componentTop
      ) {
    ...
    const actualZapasGora =
      (targetComponent.height || 0) - positionInComponent;
...
export function validateManhole(
  ...
  if (ending) {
    const supportingElement =
      components[components.indexOf(ending) - 1];
    if (supportingElement) {
      const hasReduction =
        supportingElement.componentType === 'plyta_redukcyjna';
      if (
        !hasReduction &&
        ending.dn !== supportingElement.dn
      ) {
        issues.push(
  ...
  const hasPlytaOdciazajaca = components.some(
    (c) => c.componentType === 'plyta_odciazajaca'
  );
