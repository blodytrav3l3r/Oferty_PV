...
function buildContactCell(title: string, u: UserContactInfo): TableCell {
    const runs: TextRun[] = [
        new TextRun({
            text: `${title}:`,
            bold: true,
...
export function buildContactSection(
    ...
    if (authorUser) cells.push(buildContactCell('Ofertę przygotował(-a)', authorUser));
    if (guardianUser) cells.push(buildContactCell('Opiekun handlowy (kontakt)', guardianUser));
    if (cells.length === 1) {
        cells.push(
            new TableCell({
