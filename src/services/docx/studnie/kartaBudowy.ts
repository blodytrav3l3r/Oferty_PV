...
function sectionRow(text: string, span: number = 2): TableRow {
  return new TableRow({
    children: [new TableCell({
      ...
      borders: NO_BORDERS,
      shading: { type: ShadingType.SOLID, color: COLOR_GRAY_HEADER, fill: COLOR_GRAY_HEADER },
      children: [new Paragraph({
        children: [new TextRun({ text: text.toUpperCase(), bold: true, size: SZ_TABLE_BODY, font: FONT, color: COLOR_WHITE })],
...
function infoRow(label: string, value: string, labelWidth: number = 45): TableRow {
  return new TableRow({
    children: [
      textCell(label, { bold: true, width: labelWidth, color: COLOR_LABEL, size: SZ_TABLE_BODY, borders: INFO_BOTTOM }),
...
function transitionHeaderRow(): TableRow {
  return new TableRow({
    children: TBL_COLS.map(c => textCell(c.text, {
      width: c.width, size: SZ_TABLE_HEADER, alignment: AlignmentType.CENTER,
      bold: true, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE
    }))
  });
}
...
export async function generateKartaBudowyDOCX(orderId: string): Promise<Buffer> {
    ...
    children.push(...infoSection('Logistyka i Płatności', [
    ...
    // 3. Studnia i Beton
    children.push(...infoSection('Studnia i Beton', [
      ...
      infoRow('Rodzaj stopni', String(kb.rodzajStopni || '—') + (kb.rodzajStopniInne ? ` (${kb.rodzajStopniInne})` : '')),
      infoRow('Uszczelka studni', String(kb.uszczelkaStudni || '—') + (kb.uszczelkaStudniInne ? ` (${kb.uszczelkaStudniInne})` : '')),
    ]));

    // 4. Kineta i Spocznik
    children.push(...infoSection('Kineta i Spocznik', [
      infoRow('Kineta', String(kb.kineta || '—') + (kb.kinetaInne ? ` (${kb.kinetaInne})` : '')),
      ...
      infoRow('Ślepa kineta', String(kb.slepaKineta || '—') + (kb.slepaKinetaUwagi ? ` (${kb.slepaKinetaUwagi})` : '')),
      infoRow('Kaskada', String(kb.kaskada || '—') + (kb.kaskadaUwagi ? ` (${kb.kaskadaUwagi})` : '')),
    ]));
    ...
    if (tCounts.size > 0) {
      ...
      children.push(new Table({
        ...
        rows: [
          ...
          ...sorted.map((row, idx) => new TableRow({
            children: [
          ...
          new TableRow({
            children: [
              ...
              textCell('Razem', { width: 30, size: SZ_TABLE_BODY, bold: true, alignment: AlignmentType.CENTER }),
              textCell('', { width: 14, size: SZ_TABLE_BODY }),
              textCell(String(gD), { width: 16, size: SZ_TABLE_BODY, bold: true, alignment: AlignmentType.CENTER }),
              textCell(String(gOT), { width: 16, size: SZ_TABLE_BODY, bold: true, alignment: AlignmentType.CENTER }),
              textCell(String(gTotal), { width: 16, size: SZ_TABLE_BODY, bold: true, alignment: AlignmentType.CENTER }),
            ]
          })
        ]
      }));
      ...
      children.push(new Table({
        ...
        rows: [
          ...
          new TableRow({
            children: [
              textCell('Lp.', { width: 8, size: SZ_TABLE_HEADER, alignment: AlignmentType.CENTER, bold: true, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE }),
              textCell('Indeks', { width: 22, size: SZ_TABLE_HEADER, alignment: AlignmentType.CENTER, bold: true, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE }),
              textCell('Typ elementu', { width: 20, size: SZ_TABLE_HEADER, alignment: AlignmentType.CENTER, bold: true, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE }),
              textCell('Opis', { width: 30, size: SZ_TABLE_HEADER, alignment: AlignmentType.CENTER, bold: true, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE }),
              textCell('Ilość', { width: 20, size: SZ_TABLE_HEADER, alignment: AlignmentType.CENTER, bold: true, fill: COLOR_GRAY_HEADER, color: COLOR_WHITE }),
            ]
          }),
          ...sorted.map((row, idx) => new TableRow({
            children: [
              ...
              textCell(String(row.qty), { width: 20, size: SZ_TABLE_BODY, alignment: AlignmentType.CENTER }),
            ]
          })),
          new TableRow({
            children: [
              ...
              textCell('Razem', { width: 20, size: SZ_TABLE_BODY, bold: true, alignment: AlignmentType.CENTER }),
              textCell('', { width: 30, size: SZ_TABLE_BODY }),
              textCell(String(totalQty), { width: 20, size: SZ_TABLE_BODY, bold: true, alignment: AlignmentType.CENTER }),
            ]
          })
        ]
      }));
