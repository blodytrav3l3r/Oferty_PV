...
class RuleEngine:
    ...
    def get_all_dennice_sorted(self) -> List[ProductModel]:
        """ Zwraca listę wszystkich dostępnych dennic dla danego DN (posortowanych preferencją i wysokością) """
    ...
    def get_default_manhole(self) -> Optional[ProductModel]:
        """ Szuka domyślnego włazu 15 cm lub jakiegokolwiek pierwszego włazu """
        manholes = [p for p in self.products if p.componentType == "wlaz"]
        if not manholes:
            return None
        # Prefer target thickness/height ~150mm (WLAZ-150)
        closest_150 = sorted(manholes, key=lambda p: abs(p.height - 150))
        return closest_150[0]
