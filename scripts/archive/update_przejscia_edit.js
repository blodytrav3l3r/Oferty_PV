...
// 3. Update _syncCustomRowsFromDOM to sync ALL rows (offer and custom) so we don't lose edits on re-render
...
const newSyncRowsFunc = `function _syncCustomRowsFromDOM() {
    ...
    rows.forEach(tr => {
        ...
        if (source === 'custom') {
            ...
            _modifiedOfferRows.set(idx, data);
        }
    });
}`;
...
const newCollectFunc = `function collectPrzejsciaDetailsFromTable() {
    // If the table is not rendered, return empty (or current state if we wanted to be perfectly safe, but DOM is source of truth here)
    const result = [];
...
// 5. Update renderPrzejsciaDetailsTable to apply _deletedOfferRows and _modifiedOfferRows
// We need to insert logic right after offerTypes is generated
const renderRegex = /(const offerTypes = buildOfferPrzejsciaTypes\(\);)/;
const renderInject = \`\$1
    
    // Filtrujemy usunięte z oferty
    let activeOfferTypes = offerTypes.filter(ot => !_deletedOfferRows.has(ot.rodzaj));
    
    // Aplikujemy modyfikacje
    activeOfferTypes = activeOfferTypes.map((ot, idx) => {
        if (_modifiedOfferRows.has(idx.toString())) {
            return _modifiedOfferRows.get(idx.toString());
        }
        return ot;
    });\`;
content = content.replace(renderRegex, renderInject);

// 6. Update the iteration over offerTypes in renderPrzejsciaDetailsTable to use activeOfferTypes
