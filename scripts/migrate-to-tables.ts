...
async function main() {
    ...
    const precoRow = await prisma.settings.findUnique({ where: { key: 'preco_pricing' } });
    ...
    const precoData: Record<string, unknown>[] = precoRow ? JSON.parse(precoRow.value || '[]') : [];

    console.log(`Rury: ${ruryProducts.length} produktów`);
    console.log(`Studnie: ${studnieProducts.length} produktów`);
    console.log(`PRECO: ${precoData.length} wpisów\n`);
    ...
    // ── PRECO ──
    if (precoData.length > 0 && typeof precoData[0] === 'object') {
        const input = precoData[0] as Record<string, unknown>;
        const dnSizes = ['1000', '1200', '1500', '2000', '2500'];

        for (const dnStr of dnSizes) {
            const e = input[dnStr] as Record<string, unknown> | undefined;
            if (!e) continue;
            const dnStudni = Number(dnStr);

            await prisma.precoKonfig.upsert({
                where: { dnStudni },
                update: {
            ...
            if (kinety) {
                await prisma.precoKinety.deleteMany({ where: { dnStudni } });
                for (const k of kinety) {
                    await prisma.precoKinety.create({
        ...
        // zakresy — deduplikacja między DN
        const seenKeys = new Set<string>();
        for (const dnStr of dnSizes) {
            const e = input[dnStr] as Record<string, unknown> | undefined;
            if (!e) continue;
            const rangeTypes = ['spadekKineta', 'spadekMufa', 'uniesienie', 'redukcja'];
            for (const typ of rangeTypes) {
                const arr = e[typ] as Array<{ min: number; max: number; grupy: Record<string, number> }> | undefined;
                if (!arr) continue;
                for (const r of arr) {
                    for (const [grupaDn, cena] of Object.entries(r.grupy)) {
                        const key = `${typ}|${r.min}|${r.max}|${grupaDn}`;
                        if (!seenKeys.has(key)) {
                            seenKeys.add(key);
                            await prisma.precoZakresy.create({
                                data: { typ, minVal: Number(r.min), maxVal: Number(r.max), grupaDn, cena: Number(cena) }
                            });
                        }
                    }
                }
