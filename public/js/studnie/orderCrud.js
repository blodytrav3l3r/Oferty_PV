...
window.applyPreviewLockUI = function () {
    ...
    if (!banner) {
        ...
        banner.innerHTML = `
            <div style="position:fixed; top:2rem; left:50%; transform:translateX(-50%); background:rgba(var(--slate-950-rgb), 0.95); border:2px solid var(--warn-hover); color:var(--warn-hover); padding:0.8rem 2.5rem; border-radius:40px; z-index:${LAYERS.PREVIEW_BANNER}; box-shadow:0 20px 40px rgba(var(--black-rgb), 0.6); font-weight:800; display:flex; align-items:center; gap:1.5rem; backdrop-filter:blur(10px);">
