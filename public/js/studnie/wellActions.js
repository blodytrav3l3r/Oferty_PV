    ...
    if ((well.kineta === 'preco' || well.kineta === 'precotop' || well.wkladkaOsadnikPreco === 'tak') && typeof calcPrecoPricing === 'function') {
        ...
        if (precoCalc.suma > 0 || precoCalc.error) {
            ...
            if (precoCalc.error) {
                ...
                html += `<span>Kineta bazowa${bazowaLabel}</span><span style="text-align:right; font-weight:600;">${fmtInt(precoCalc.bazowa)} PLN</span>`;
                ...
                if (precoCalc.uniesieniaSzczegoly && precoCalc.uniesieniaSzczegoly.length > 0) {
                    precoCalc.uniesieniaSzczegoly.forEach(u => {
                        if (!u._wyrenderowane) {
                            ...
                            html += `<span>Uniesienie kinety (${u.mm} mm)${uLabel}</span><span style="text-align:right; font-weight:600;">${fmtInt(u.cena)} PLN</span>`;
                        }
                    });
                } else if (precoCalc.uniesienie > 0 && (!precoCalc.uniesieniaSzczegoly || precoCalc.uniesieniaSzczegoly.length === 0)) {
                    html += `<span>Uniesienie kinety</span><span style="text-align:right; font-weight:600;">${fmtInt(precoCalc.uniesienie)} PLN</span>`;
                }

                if (precoCalc.skrzynki && precoCalc.skrzynki.ilosc > 0) {
                    html += `<span>Skrzynki włazowe (${precoCalc.skrzynki.ilosc} szt.)</span><span style="text-align:right; font-weight:600;">${fmtInt(precoCalc.skrzynki.suma)} PLN</span>`;
                }
                if (precoCalc.spadkiSzczegoly && precoCalc.spadkiSzczegoly.length > 0) {
                    precoCalc.spadkiSzczegoly.forEach(s => {
                        if (!s._wyrenderowane) {
                            const sLabel = s.label ? ` [${s.label}]` : '';
                            html += `<span>Spadek ${s.typ} (${s.procent} %)${sLabel}</span><span style="text-align:right; font-weight:600;">${fmtInt(s.cena)} PLN</span>`;
                        }
                    });
                } else {
                    if (precoCalc.spadekKineta > 0) {
                        html += `<span>Spadek kineta</span><span style="text-align:right; font-weight:600;">${fmtInt(precoCalc.spadekKineta)} PLN</span>`;
                    }
                    if (precoCalc.spadekMufa > 0) {
                        html += `<span>Spadek mufa</span><span style="text-align:right; font-weight:600;">${fmtInt(precoCalc.spadekMufa)} PLN</span>`;
                    }
                }
                if (precoCalc.pelnaWysokosc) {
                    html += `<span>↳ Wkładka uzupełniająca (${precoCalc.pelnaWysokosc.metry.toFixed(2)} m)</span><span style="text-align:right; font-weight:600;">${fmtInt(precoCalc.pelnaWysokosc.cena)} PLN</span>`;
                }
                if (discPreco > 0) {
                    html += `<span style="color:#34d399;">Rabat wkładka PRECO (${discPreco}%)</span><span style="text-align:right; font-weight:600; color:#34d399;">-${fmtInt(precoCalc.suma - precoFinal)} PLN</span>`;
                }
                html += `</div></div>`;
            }
        }
