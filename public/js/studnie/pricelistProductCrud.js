...
async function copyStudnieProduct(id) {
    ...
    if (typeof invalidateStudnieProductsMap === 'function') invalidateStudnieProductsMap();
    _studniePricelistDirty = true;
    updateStudnieSaveBtn();
    renderStudniePriceList();
...
function showAddStudnieProductModal() {
    ...
    <div class="modal">
      ...
      <div class="form-group"><label class="form-label">Kategoria</label>
        ...
        <datalist id="przejscia-cats-list">
            ${[
                ...
                .filter(Boolean)
                .map((c) => `<option value="${escapeHtmlAttr(c)}">`)
                .join('')}
        </datalist>
      </div>
      ...
      <div class="form-row non-przejscia grid-3-col" >
    ...
    // Enter → Dodaj element (każde pole w modalu, poza textarea — tu brak textarea)
    setTimeout(() => {
        const handler = () => window.addStudnieProduct && window.addStudnieProduct();
