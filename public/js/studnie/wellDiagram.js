...
function drawPrecoInsertLine(well, canvas) {
    ...
    if (lineY >= mT && lineY <= mT + drawH) {
        ...
        svg += `<line x1="${xStart}" y1="${lineY}" x2="${xEnd}" y2="${lineY}" stroke="#ef4444" stroke-width="1.5" stroke-dasharray="6,3" opacity="0.85"/>`;
        // Mały trójkąt / strzałka wskazująca na linię z lewej
        svg += `<polygon points="${xStart - 3},${lineY - 3} ${xStart - 3},${lineY + 3} ${xStart},${lineY}" fill="#ef4444" opacity="0.85"/>`;
        // Etykieta wewnątrz grafiki po prawej stronie
        svg += `<text x="${rightEdge - 4}" y="${lineY + 10}" text-anchor="end" fill="#ef4444" font-size="8" font-family="Inter,sans-serif" font-weight="700" opacity="0.9">▼ ${kinetaLabel}</text>`;
    }
    ...
    if (well.precoFullHeight === 'tak' || well.precoFullHeight === true) {
        ...
        if (dennicaHeight > insertHeightMm) {
            ...
            if (lineYFull >= mT && lineYFull <= mT + drawH) {
                ...
                svg += `<line x1="${xStart}" y1="${lineYFull}" x2="${xEnd}" y2="${lineYFull}" stroke="#f59e0b" stroke-width="1.5" stroke-dasharray="4,2" opacity="0.9"/>`;
                svg += `<polygon points="${xStart - 3},${lineYFull - 3} ${xStart - 3},${lineYFull + 3} ${xStart},${lineYFull}" fill="#f59e0b" opacity="0.9"/>`;
                svg += `<text x="${rightEdge - 4}" y="${lineYFull + 10}" text-anchor="end" fill="#f59e0b" font-size="8" font-family="Inter,sans-serif" font-weight="700" opacity="0.9">▼ Uzupełnienie do pełnej wysokości</text>`;
            }
        }
    }
