    ...
    tabWells.forEach((well, idx) => {
        ...
        for (let i = 0; i < maxTr; i++) {
            ...
            var typeHtml = _excelOverlaySelectHtml(
                catOpts2,
                activeCategory,
                'excelOnPrzejscieTypeChange(' + wIdx + ',' + i + ',this.value)',
                120
            );
            ...
            var dnHtml = _excelOverlaySelectHtml(
                dnOpts2,
                prz.productId,
                'excelOnPrzejscieChange(' + wIdx + ',' + i + ",'productId',this.value)",
                110
            );
        ...
        /* Psia buda */
        html += `<td style="${tdBase}text-align:center;"><input type="checkbox"${well.psiaBuda ? ' checked' : ''} onchange="excelOnPsiaBudaChange(${wIdx},this.checked)" style="accent-color:#f59e0b;cursor:pointer;transform:scale(1.3);" /></td>`;
