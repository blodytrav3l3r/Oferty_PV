...
function _displayTransportCost(tCost, costPerTrip) {
    ...
    if (t > 0 && costPerTrip > 0) {
        ...
        wyliczonyTransportInput.value = `${countLabel} x ${fmt(costPerTrip)} zĹ‚ = ${fmt(t)} zĹ‚`;
    } else if (t > 0) {
        wyliczonyTransportInput.value = `${fmt(t)} zĹ‚`;
    } else {
...
function _detectWellParams() {
    ...
    if (hasZelbet) result.rodzajStudni = 'Ĺ»elbet';
    ...
    selectedWells.forEach((w) => {
        if (w.malowanieW && w.malowanieW !== 'brak') {
            const price = parseFloat(w.malowanieWewCena || 0)
                .toFixed(2)
                .replace('.', ',');
            paintingLines.add(`Malowanie wewnÄ…trz (${w.malowanieW}): ${price} PLN/mÂ˛`);
        }
        if (w.malowanieZ && w.malowanieZ !== 'brak') {
            const price = parseFloat(w.malowanieZewCena || 0)
                .toFixed(2)
                .replace('.', ',');
            paintingLines.add(`Malowanie zewnÄ…trz (${w.malowanieZ}): ${price} PLN/mÂ˛`);
        }
    });

    precoDiscounts.forEach((pre) => lines.push(`Preco: ${pre.toFixed(2).replace('.', ',')}%`));
    pehdDiscounts.forEach((pehd) =>
        lines.push(`WkĹ‚adka PEHD: ${pehd.toFixed(2).replace('.', ',')}%`)
    );
...
async function showKartaBudowyCopyPicker() {
    ...
    if (!orders.some((order) => order.kartaBudowy)) {
        if (typeof showToast === 'function') {
            showToast('Brak zamĂłwieĹ„ z zapisanÄ… KartÄ… Budowy dla tej oferty.', 'info');
        }
        return;
    }
...
function renderKartaBudowyCopyOptions() {
    ...
    if (!orders || orders.length === 0) {
        copySelect.innerHTML = '<option>Brak zamĂłwieĹ„ do kopiowania</option>';
        copySelect.disabled = true;
        if (copyButton) copyButton.disabled = true;
        if (helpText)
            helpText.textContent = 'Brak wczeĹ›niejszych zamĂłwieĹ„ powiÄ…zanych z tÄ… ofertÄ….';
        return;
    }

    const optionsHtml = [`<option value="">Wybierz kartÄ™ budowy do skopiowania</option>`].concat(
        orders.map((order) => {
    ...
    if (helpText)
        helpText.textContent =
            'Wybierz istniejÄ…ce zamĂłwienie, aby skopiowaÄ‡ jego dane Karty Budowy.';
}
...
function copyKartaBudowyFromOrder() {
    ...
    if (!orderId) {
        if (typeof showToast === 'function') {
            showToast('Wybierz zamĂłwienie do skopiowania.', 'error');
        }
        return;
    }
    ...
    if (!sourceOrder) {
        if (typeof showToast === 'function') {
            showToast('Nie znaleziono wybranego zamĂłwienia.', 'error');
        }
        return;
    }
    ...
    if (!sourceOrder.kartaBudowy) {
        if (typeof showToast === 'function') {
            showToast('Wybrane zamĂłwienie nie ma zapisanych danych Karty Budowy.', 'error');
        }
        return;
    }
    ...
    if (typeof showToast === 'function') {
        showToast(
            `Skopiowano dane Karty Budowy z zamĂłwienia ${sourceOrder.orderNumber || sourceOrder.id.substring(0, 8)}.`,
            'success'
        );
    }
}
