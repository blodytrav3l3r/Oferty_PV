...
function openExcelShortcutsPopup() {
    ...
    const legend =
        ...
        '</div><span style="color:var(--excel-text-dim);">Priorytet: <b>duplikat &gt; ERROR &gt; WARNING &gt; aktywny</b> — przy konflikcie cały wiersz ma tło duplikatu, a czcionka wskazuje ERROR/WARNING.</span></span>' +
        '<span style="width:16px;height:12px;border-radius:2px;background:rgba(var(--blue-rgb), 0.24);border:1px solid rgba(var(--blue-rgb), 0.3);display:inline-block;margin-top:2px;"></span><span><b>Aktywny wiersz</b> <span style="color:var(--excel-text-dim);">tło var(--excel-row-active), hover 0.28</span><br><span style="color:var(--excel-text-dim);">Kiedy:</span> <code>wIdx === currentWellIndex</code> — klik/fokus w wierszu; podświetla studnię pokazywaną w diagramie po lewej. Z duplikatem: <code>rgba(dup 0.3)</code>, hover <code>0.35</code>.</span>' +
        ...
        '</div>' +
        '<p class="sc-legend-note">Kolory liczone w <code>excelTableBody.js:_excelRenderTbody</code> i odświeżane bez re-renderu w <code>_excelRefreshDupColors</code>; statusy z <code>solverValidation.js:recalculateWellErrors()</code> przez polling. Priorytet tła: <b>duplikat &gt; ERROR (0.12) &gt; WARNING (0.1) &gt; aktywny (0.24 w light, 0.18 w dark) &gt; naprzemienny</b>.</p>' +
        '</div>';
    const html =
