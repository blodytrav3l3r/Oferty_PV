...
window.handleOfferTransportSave = async function () {
    ...
    if (typeof window.appConfirm === 'function') {
        const inOrderMode = typeof orderEditMode !== 'undefined' && orderEditMode;
        const confirmed = await window.appConfirm(
            `<div class="fs-3xl-eb">Zapisz nową konfigurację transportu</div>
             <div class="fs-xl">Czy na pewno chcesz zapisać parametry przewozu do ${inOrderMode ? 'zamówienia' : 'oferty'}?</div>`,
            {
                allowHtml: true,
