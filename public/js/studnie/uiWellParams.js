...
function renderWellParams() {
    ...
    let html = `<div style="display:flex; align-items:center; justify-content:space-between; gap:0.6rem; margin-bottom:0.6rem; padding:0.35rem 0.5rem; background:rgba(var(--slate-800-rgb),0.5); border:1px solid rgba(var(--white-rgb),0.05); border-radius:var(--radius-sm);">
        ...
        <button type="button" class="btn btn-sm ${hasUwagiCurrent ? 'has-uwagi' : ''}" onclick="openWellNotesForCurrent()" title="${hasUwagiCurrent ? escapeHtmlAttr(String(well.uwagi).slice(0, 80)) : 'Dodaj uwagi do tej studni'}" style="${hasUwagiCurrent ? 'background:rgba(var(--warn-rgb),0.15); border-color:rgba(var(--warn-rgb),0.3); color:var(--warn-hover);' : ''}"><i data-lucide="file-text" class="icon-xs" aria-hidden="true"></i> Uwagi${hasUwagiCurrent ? ' •' : ''}</button>
    </div>`;
