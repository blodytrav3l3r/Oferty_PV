...
function mergeOverlappingRanges(ranges) {
    ...
    var sorted = ranges.slice().sort(function (a, b) { return a.bottom - b.bottom; });
...
function calcPrecoPricingPure(well, helpers) {
    ...
    function emptyResult() {
        return {
            bazowa: 0, dodWloty: [], spadekKineta: 0, spadekMufa: 0,
            uniesienie: 0, redukcja: 0,
    ...
    if (well.wkladkaOsadnikPreco === 'tak') {
        ...
        if (heightMm > 0) {
            result.pelnaWysokosc = { metry: heightMm / 1000, cena: heightCost, startZ: 0, endZ: heightMm };
        }
    ...
    var allPipes = (well.przejscia || []).map(function (p, index) {
        var prod = null;
        for (var si = 0; si < studnieProducts.length; si++) {
            if (studnieProducts[si].id === p.productId) { prod = studnieProducts[si]; break; }
        }
        return {
            ...p,
    ...
    }).filter(function (p) { return p.dnRury > 0; });
    ...
    var getZeroScore = function (kat) { return Math.min(Math.abs(kat), Math.abs(kat - 360)); };
    ...
    var przejscia = mainPipes.concat(doloty.sort(function (a, b) { return b.dnRury - a.dnRury; }));
    ...
    var rangesForMerge = przejscia.map(function (p) { return { bottom: p._mmFromBottom, top: p._goraPrzejscia }; });
    ...
    for (var di = 2; di < przejscia.length; di++) {
        ...
        if (mmFromBottomD >= precoInsertTop) {
            ...
            for (var ok = 0; ok < przejscia.length; ok++) {
                ...
                if (goraOther < goraPrzejsciaD) { isKaskada = true; break; }
            }
            var dodRow = null;
            for (var dr = 0; dr < cennik.kinety.length; dr++) {
                if (cennik.kinety[dr].dn >= dp.dnRury) { dodRow = cennik.kinety[dr]; break; }
            }
            if (dodRow) {
                var typ = isKaskada ? 'kaskada' : 'sciana';
                result.dodWloty.push({ _id: dp._oryginalnyIndex, dn: dp.dnRury, cena: dodRow.dodWlot, typ: typ, label: dp._flowLabel });
            }
        } else {
            var dodRow2 = null;
            for (var dr2 = 0; dr2 < cennik.kinety.length; dr2++) {
                if (cennik.kinety[dr2].dn >= dp.dnRury) { dodRow2 = cennik.kinety[dr2]; break; }
            }
            if (dodRow2) {
                result.dodWloty.push({ _id: dp._oryginalnyIndex, dn: dp.dnRury, cena: dodRow2.dodWlot, typ: 'doplyw', label: dp._flowLabel });
            }
        }
    }
    ...
    if (mainPipes[0].dnRury >= 500 && cennik.skrzynkaWlazowa) {
        ...
        result.skrzynki = { ilosc: ilosc, cenaSzt: cennik.skrzynkaWlazowa, suma: ilosc * cennik.skrzynkaWlazowa };
    }
    ...
    przejscia.forEach(function (pp) {
        if (pp.spadekKineta) {
            ...
            if (kwota > 0) {
                ...
                result.spadkiSzczegoly.push({ _id: pp._oryginalnyIndex, label: pp._flowLabel, typ: 'kinety', procent: pp.spadekKineta, cena: kwota });
            }
        }
        if (pp.spadekMufa) {
            ...
            if (kwota2 > 0) {
                ...
                result.spadkiSzczegoly.push({ _id: pp._oryginalnyIndex, label: pp._flowLabel, typ: 'mufy', procent: pp.spadekMufa, cena: kwota2 });
            }
        }
    });
    ...
    // 8. Redukcja kinety
    if (well.redukcjaKinety !== 'nie' && mainPipes.length > 1 && mainPipes[0].dnRury !== mainPipes[1].dnRury && cennik.redukcja) {
        var roznicaSrednic = Math.abs(mainPipes[0].dnRury - mainPipes[1].dnRury);
        result.redukcja = _findPrecoRange(cennik.redukcja, roznicaSrednic, Math.max(mainPipes[0].dnRury, mainPipes[1].dnRury));
    ...
    if ((well.precoFullHeight === 'tak' || well.precoFullHeight === true) && cennik.cenaPelnaWysMB) {
        ...
        if (well.config) {
            for (var ci = 0; ci < well.config.length; ci++) {
                ...
                for (var si2 = 0; si2 < studnieProducts.length; si2++) {
                    if (studnieProducts[si2].id === item.productId) { prod2 = studnieProducts[si2]; break; }
                }
                if (prod2 && (prod2.componentType === 'dennica' || prod2.componentType === 'styczna')) {
        ...
        if (pozostaloMm > 0) {
            ...
            result.pelnaWysokosc = { metry: metry, cena: metry * cennik.cenaPelnaWysMB, startZ: precoInsertTop, endZ: dennicaHeight };
        }
    }

    // Suma
    result.suma = result.bazowa
        + result.dodWloty.reduce(function (s, d) { return s + d.cena; }, 0)
