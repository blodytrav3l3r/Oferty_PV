        ...
        <div class="flex-gap-5-wrap2">
            <div>
                <div style="font-size: var(--fs-md); font-weight: var(--fw-extrabold); color:${hasChanges ? 'var(--danger-hover)' : 'var(--success-hover)'}; display:flex; align-items:center; gap:0.4rem;">
                    <i data-lucide="package" class="icon-md"></i> TRYB ZAMÓWIENIA — ${escapeHtml(order.orderNumber || order.number || '')}
                </div>
                <div class="fs-xs-muted">
                    Oferta: ${escapeHtml(order.offerNumber || '—')} •
                    ${hasChanges ? `<i data-lucide="alert-triangle" class="icon-xs"></i> ${changeCount > 0 ? `${changeCount} studni zmienionych od oryginału` : ''}${changeCount > 0 && transportChanged ? ' • ' : ''}${transportChanged ? 'zmieniono transport' : ''}` : '<i data-lucide="check-circle-2" class="icon-xs"></i> Bez zmian od oryginału'}
