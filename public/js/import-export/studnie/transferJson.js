window.StudnieTransferJson = {
    ...
    async exportOrder(orderId) {
        const order = await JsonOfferTransfer.fetchOrder('studnie', orderId);
        const payload = JsonOfferTransfer.buildOrderPayload('studnie', order);
        ...
        JsonOfferTransfer.downloadFile(
            payload,
            'zamowienie_studnie_' + safeNumber + '_' + date + '.json'
        );
        ...
        await fetch('/api/feature-flags/audit', {
            ...
            body: JSON.stringify({
                ...
                entityId: orderId,
                action: 'export.transfer',
                details: { module: 'studnie', orderNumber: safeNumber }
            })
        });
    },
    ...
    async importOrder(file) {
        ...
        const orderPayload = {
            id: order.id,
            offerStudnieId: order.offerStudnieId || order.offerId,
            status: order.status || 'new',
            createdAt: order.createdAt,
            ...Object.fromEntries(
        ...
        try {
            const resp = await fetch('/api/orders-studnie', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include',
                body: JSON.stringify({ data: [orderPayload] })
            ...
            if (!resp.ok) {
                const err = await resp.json();
                throw new Error(err.error || 'Blad importu zamowienia studni');
            }
