...
function initKartaBudowyStep4(primaryOfferNumber) {
    ...
    // Wyliczony transport — deleguj do updateTransportCostSummary (respektuje tryb full/fractional)
    ...
    // Auto-uzupełnij uwagi ogólne listą rabatów + zabezpieczeniem transportu
    ...
    if (uwagiField && activeItemsForUwagi && activeItemsForUwagi.length > 0) {
        ...
        if (autoLines.length > 0) {
            ...
            if (isOrder) {
                // W trybie zamówienia: usuń starą sekcję auto-generated z końca i nadpisz nową (tylko z elementów zamówienia)
 ...
 *  Porównuje ceny z transportem (unitTotal = priceAfterDiscount + transportPerUnit).
 *  Przy zmianie transportu (km/rate/mode) — oznacz wszystkie elementy jako zmienione.
 *  Analogicznie do getOrderChanges w studnie. */
