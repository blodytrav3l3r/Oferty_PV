...
/* buildOfferXlsxRows, buildOfferXlsxMetaRows z offerExportsHelpers.js */
...
function exportOfferPDF(id) {
    ...
    <table><thead><tr><th>Produkt</th><th class="text-right">Ilość</th><th class="text-right">Waga/szt</th><th class="text-right">Łączna waga</th><th class="text-right">Max/transport</th><th class="text-right">Transporty</th></tr></thead><tbody>`;
        ...
        if (transportResult.saved > 0 && offerMode !== 'fractional') {
            transportHtml += `<div style="font-size: var(--fs-sm);color:var(--success);margin-top:4px">Optymalizacja: połączono niepełne transporty (${transportResult.lines.reduce((s, l) => s + l.dedicatedTransports, 0)} → ${transportResult.totalTransports})</div>`;
        }
    }
  ...
  <style>
    ${PRINT_TOKENS_CSS}
    body{font-family:Arial,sans-serif;color:var(--slate-950);padding:30px;font-size: var(--fs-md);line-height:1.5}
    h1{font-size: var(--fs-4xl);color:var(--brand-navy);margin-bottom:5px}
    .header-line{border-bottom:3px solid var(--brand-navy);padding-bottom:10px;margin-bottom:15px}
    .info-grid{display:grid;grid-template-columns:1fr 1fr;gap:15px;margin-bottom:20px}
    .info-box{background:var(--slate-100);border-radius: var(--radius-sm);padding:12px}
    .info-box h3{font-size: var(--fs-base);color:var(--slate-500);text-transform:uppercase;margin-bottom:5px}
    table{width:100%;border-collapse:collapse;margin:15px 0;font-size: var(--fs-base)}
    th{background:var(--brand-navy);color:var(--white);padding:8px;text-align:left;font-size: var(--fs-sm)}
    td{padding:7px 8px;border-bottom:1px solid var(--slate-200)}
    tr:nth-child(even){background:var(--slate-50)}
    .text-right{text-align:right}
    .summary{background:var(--slate-100);border-radius: var(--radius-sm);padding:15px;margin-top:15px}
    .summary-row{display:flex;justify-content:space-between;padding:4px 0}
    .summary-row.total{font-weight:bold;font-size: var(--fs-xl);border-top:2px solid var(--brand-navy);padding-top:8px;margin-top:5px}
    .summary-row.transport{color:var(--warn)}
    .notes{margin-top:15px;padding:10px;background:var(--warn-bg-light);border-left:3px solid var(--warn);border-radius: var(--radius-2xs);font-size: var(--fs-base)}
    .footer{margin-top:10px;font-size: var(--fs-sm);color:var(--slate-500);text-align:center;padding-top:10px}
  ...
  <div style="margin-top: 15px; font-size: var(--fs-sm);">
...
function renderDiscountModalItems() {
    ...
    sortedItems.forEach(({ item, index }) => {
        ...
        const warningText = isGasket
            ? '<div style="font-size: var(--fs-xs); color:var(--danger); font-weight: var(--fw-bold); margin-top:4px; line-height:1.2;">Uwaga rabat<br>na uszczelki !</div>'
            : '';
      ...
      <tr style="border-bottom:1px solid var(--border-glass);">
        ...
        <td id="modal-netto-${index}" style="padding:0.4rem; text-align:right; font-weight: var(--fw-bold); color:var(--text-primary); font-size: var(--fs-xl);">${fmt(netto)} PLN</td>
      </tr>
    `;
    });
    ...
    const xlsxTransports =
        xlsxMode === 'fractional'
            ? offer.items
                  ...
                  .reduce((s, i) => s + i.weight * i.quantity, 0) / MAX_TRANSPORT_WEIGHT
            : transportResult.totalTransports;
    const { rows, headerRow } = buildOfferXlsxRows(
        offer,
        transportDist,
        costPerTrip,
        xlsxTransports
    );
    const metaRows = buildOfferXlsxMetaRows(offer);
