...
(async () => {
    ...
    console.log(
        'spa-main children:',
        await page.evaluate(() => document.getElementById('spa-main')?.children.length)
    );
    ...
    console.log(
        'getAuthToken:',
        await page.evaluate(() => (typeof getAuthToken === 'function' ? 'shim-ok' : 'brak funkcji'))
    );
    ...
    console.log(
        ...
        await page.evaluate(() => {
            const s = document.getElementById('wizard-step-1');
            return s ? getComputedStyle(s).display : 'BRAK ELEMENTU';
        })
    );
    console.log(
        'client-name input:',
        await page.evaluate(() => !!document.getElementById('client-name'))
    );
