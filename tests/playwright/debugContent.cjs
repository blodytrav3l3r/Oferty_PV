...
function resolvePlaywright() {
    ...
    for (const root of roots) {
        try {
            const hashes = readdirSync(root, { withFileTypes: true }).filter((d) =>
                d.isDirectory()
            );
            for (const h of hashes) {
                try {
                    return require(join(root, h.name, 'node_modules', 'playwright'));
                } catch (_) {}
            }
        } catch (_) {}
    }
...
(async () => {
    ...
    for (const mod of ['studnie', 'rury', 'kartoteka', 'zlecenia']) {
        ...
        // Wariant A: login przez Node fetch (osobny jar) — cookie do kontekstu
        // przez addCookies; localStorage nieużywany.
                ...
                info = await frame
                    .evaluate(() => {
                        ...
                        return JSON.stringify({
                            ...
                            offersListLen: document.getElementById('ka-offers-list')
                                ? document.getElementById('ka-offers-list').innerText.length
                                : null,
                            zleceniaRows: document.getElementById('zlecenia-table-body')
                                ? document.getElementById('zlecenia-table-body').children.length
                                : null,
                            partialHosts: Array.from(document.querySelectorAll('[data-partial]'))
                                .map((h) => `${h.id}:${h.children.length}`)
                                .join(','),
