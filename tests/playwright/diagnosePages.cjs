    ...
    for (const mod of MODULES) {
        ...
        page.on('console', (msg) => {
            if (msg.type() === 'error' || msg.type() === 'warning')
                problems.push(`[console.${msg.type()}] ${msg.text().slice(0, 220)}`);
        });
        ...
        const frameEl = await page
            .waitForSelector(`#spa-iframe-${mod}`, { timeout: 10000 })
            .catch(() => null);
        let bodyLen = -1;
        if (frameEl) {
            ...
            if (frame) {
                ...
                const sections = await frame
                    .evaluate(() =>
                        Array.from(document.querySelectorAll('.section.active, .section')).map(
                            (s) => `${s.id}:${s.innerText.trim().length}`
                        )
                    )
                    .catch(() => []);
