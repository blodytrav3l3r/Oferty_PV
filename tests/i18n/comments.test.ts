...
const ALLOWED_WORDS: Record<string, string[]> = {
  'public/css/style.css': ['Toast', 'flex', 'layout', 'inline', 'Linear', 'admin', 'menu', 'Basic', 'slide', 'fade', 'spin', 'pulse', 'none', 'auto', 'sticky', 'scroll'],
  'public/css/studnie.css': ['flex', 'layout', 'inline', 'Toast', 'Menu', 'admin', 'slide', 'none', 'auto', 'scroll', 'primary'],
  'public/css/zlecenia.css': ['Toast', 'flex', 'none', 'auto'],
  'public/css/index.css': ['Toast', 'flex', 'admin', 'inline', 'none', 'auto', 'scroll', 'slide', 'pulse', 'primary'],
};
...
const ENGLISH_WORDS = [
  'Header', 'Navigation', 'Button', 'Input',
  'Search', 'Footer',
  'Wrapper', 'Section', 'Container',
  'Filter', 'Tab', 'Responsive', 'Breakpoint',
  'Hover', 'Focus', 'Active',
  ...
  'Delete', 'Save',
  'Info', 'Error', 'Warning', 'Success', 'Danger',
  'Light', 'Dark', 'Fixed', 'Hidden', 'Visible',
  'Block',
];
    ...
    for (const comment of comments) {
      for (const word of ENGLISH_WORDS) {
        ...
        if (regex.test(comment)) {
          failures.push(`Znaleziono "${word}" w komentarzu: "${comment.trim().substring(0, 80)}..."`);
        }
      }
    }
