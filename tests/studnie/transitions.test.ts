...
interface Segment { type: string; start: number; end: number; itemBase?: any; }
...
describe('Transitions — dennica selection', () => {
    it('100mm → physical OK, D-300 (top=40>=0)', () => {
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-160', rzednaWlaczenia: 0.1 }], 0);
    ...
    it('300mm → D-300 skipped (300>=300), D-300 wins', () => {
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-160', rzednaWlaczenia: 0.3 }], 0);
        expect(d!.id).toBe('D-1000-300');
    });

    it('powyżej dennicy (800mm) → wszystkie skipped → D-300', () => {
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-160', rzednaWlaczenia: 0.8 }], 0);
        expect(d!.id).toBe('D-1000-300');
    });

    it('DN200@100mm → D-300 physical OK (top=0>=0)', () => {
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-200', rzednaWlaczenia: 0.1 }], 0);
        expect(d!.id).toBe('D-1000-300');
    });
});
...
describe('Transitions — różne rzędne dna', () => {
    it('rzDna=100, rzedna=100.1 → z=100mm → D-300 physical OK', () => {
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-160', rzednaWlaczenia: 100.1 }], 100);
        expect(d!.id).toBe('D-1000-300');
    });
    ...
    it('rzDna=100, rzedna=100.3 → z≈300mm (FP), D-500 physical OK', () => {
        ...
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-160', rzednaWlaczenia: 100.3 }], 100);
        expect(d!.id).toBe('D-1000-500');
    });

    it('rzedna == rzDna → hcInvert=0 → nearBottom, D-500 standard OK (top=340>=315)', () => {
        const d = selectLowestDennica(ALL, 1000, 'Kluczbork', [{ productId: 'PRZ-160', rzednaWlaczenia: 50 }], 50);
        expect(d!.id).toBe('D-1000-500');
    });
});
...
describe('Transitions — OT ring substitution', () => {
    function buildSegments(configItems: Array<{ productId: string; quantity: number }>, products: MockProduct[]): Segment[] {
        let y = 0;
    ...
    function needsOT(segments: Segment[], transitions: Transition[], products: MockProduct[], rzDna: number): boolean {
    ...
    const KREG = { id: 'K-1000-1000', name: 'Krag 1000', componentType: 'krag', dn: 1000, height: 1000, formaStandardowaKLB: 1 };
    const KREG500 = { id: 'K-1000-500', name: 'Krag 500', componentType: 'krag', dn: 1000, height: 500, formaStandardowaKLB: 1 };

    it('przejście w kręgu → potrzebny OT', () => {
        // Dennica 500 + krag 1000
        // Przejście DN160@700mm → środek=780 → w kręgu (500-1500)
        const segs = buildSegments(
            [{ productId: 'D-1000-500', quantity: 1 }, { productId: 'K-1000-1000', quantity: 1 }],
            [...DENNICE, KREG]
        );
        expect(needsOT(segs, [{ productId: 'PRZ-160', rzednaWlaczenia: 0.7 }], [PRZ], 0)).toBe(true);
    });
    ...
    it('przejście w dennicy → OT niepotrzebny', () => {
        ...
        expect(needsOT(segs, [{ productId: 'PRZ-160', rzednaWlaczenia: 0.2 }], [PRZ], 0)).toBe(false);
    });
    ...
    it('przejście powyżej wszystkich kręgów → OT niepotrzebny', () => {
        ...
        expect(needsOT(segs, [{ productId: 'PRZ-160', rzednaWlaczenia: 2.0 }], [PRZ], 0)).toBe(false);
    });
    ...
    it('dwa kręgi → przejście w drugim kręgu', () => {
        ...
        expect(needsOT(segs, [{ productId: 'PRZ-160', rzednaWlaczenia: 1.2 }], [PRZ], 0)).toBe(true);
    });

    it('przejście na granicy segmentów → środek decyduje', () => {
        const segs = buildSegments(
            [{ productId: 'D-1000-500', quantity: 1 }, { productId: 'K-1000-500', quantity: 1 }],
            [...DENNICE, KREG500]
        );
        // Przejście DN160@450mm → środek=530 → w kręgu (500-1000)
        expect(needsOT(segs, [{ productId: 'PRZ-160', rzednaWlaczenia: 0.45 }], [PRZ], 0)).toBe(true);
    });
});
...
describe('Transitions — conflict with segments', () => {
    ...
    it('przejście w dennicy → OK gdy seg.end poza strefą standard', () => {
        ...
        const segs: Segment[] = [{ type: 'dennica', start: 0, end: 700 }, { type: 'krag', start: 700, end: 1500 }];
        const r = checkConflictsLogic([{ ...BH, z: 200 }], segs);
        expect(r.valid).toBe(true);
    });

    it('przejście w kręgu → OK', () => {
        const r = checkConflictsLogic([{ ...BH, z: 700, zdD: 300, zdG: 200 }],
    ...
    it('przejście z dużym DN → szersza strefa kolizji', () => {
        ...
        const r = checkConflictsLogic([{ z: 700, ruraDz: 200, zdD: 300, zdG: 300, zdDM: 150, zdGM: 150 }],
            [{ type: 'dennica', start: 0, end: 500 }, { type: 'krag', start: 500, end: 1500 }]);
    ...
    it('przejście na granicy konusa → kolizja', () => {
        const r = checkConflictsLogic([{ z: 1400, ruraDz: 160, zdD: 300, zdG: 300, zdDM: 150, zdGM: 150 }], SEGS);
    ...
    it('wiele przejść → łączna walidacja', () => {
        ...
        const segs: Segment[] = [{ type: 'dennica', start: 0, end: 800 }, { type: 'krag', start: 800, end: 2000 }];
...
describe('Transitions — complete well config scenarios', () => {
    ...
    it('niestandardowe zapasy (500/200) → D-400 physical OK (top=40>=0)', () => {
        const PRZ_CUSTOM: MockProduct = {
            ...
            zapasDol: 500, zapasGora: 500, zapasDolMin: 200, zapasGoraMin: 200, formaStandardowaKLB: 1,
        };
        // bottom=200 < 200+15=215 → all fail minimal → physical
        // D-400: top=400-(200+160)=40>=0 → physical OK → D-400
        const d = selectLowestDennica([...DENNICE, PRZ_CUSTOM], 1000, 'Kluczbork',
            [{ productId: 'PRZ-CUSTOM', rzednaWlaczenia: 0.2 }], 0);
