...
function buildProductCatalog(): MockProduct[] {
  ...
  for (const dn of [1000, 1200, 1500, 2000]) {
    const prefix = dn === 1000 ? 'KON-10' : dn === 1200 ? 'KON-12' : dn === 1500 ? 'KON-15' : 'KON-20';
  ...
  for (const [dn, heights] of Object.entries(RING_HEIGHTS)) {
    const prefix = dn === '1000' ? 'KDB-10' : dn === '1200' ? 'KDB-12' : dn === '1500' ? 'KDB-15' : 'KDB-20';
    for (const h of heights) {
      products.push({
  ...
  const przejsciaDefs: ProdDef[] = [
    ...
    { id: 'PRZ-160', name: 'Przejście 160', componentType: 'przejscie', dn: '200/160', height: 0, zapasDol: 300, zapasGora: 300, zapasDolMin: 150, zapasGoraMin: 150 },
    { id: 'PRZ-200', name: 'Przejście 200', componentType: 'przejscie', dn: '250/200', height: 0, zapasDol: 300, zapasGora: 300, zapasDolMin: 150, zapasGoraMin: 150 },
    { id: 'PRZ-300', name: 'Przejście 300', componentType: 'przejscie', dn: '355/300', height: 0, zapasDol: 300, zapasGora: 300, zapasDolMin: 150, zapasGoraMin: 150 },
    // Tight clearances (100/100, min 50/50)
    { id: 'PRZ-160-T', name: 'Przejście 160 ciasne', componentType: 'przejscie', dn: '200/160', height: 0, zapasDol: 100, zapasGora: 100, zapasDolMin: 50, zapasGoraMin: 50 },
    { id: 'PRZ-200-T', name: 'Przejście 200 ciasne', componentType: 'przejscie', dn: '250/200', height: 0, zapasDol: 100, zapasGora: 100, zapasDolMin: 50, zapasGoraMin: 50 },
    // Minimal-only (50/50, min 30/30)
    { id: 'PRZ-160-M', name: 'Przejście 160 minimal', componentType: 'przejscie', dn: '200/160', height: 0, zapasDol: 50, zapasGora: 50, zapasDolMin: 30, zapasGoraMin: 30 },
  ];
...
function selectLowestDennica(
  ...
  const dennicy = products
    .filter((p) => {
      ...
      return p.componentType === 'dennica' && parseInt(String(p.dn)) === parseInt(String(dn)) && (p.height ?? 0) > 0;
    })
    .sort((a, b) => {
      const hA = a.height || 0;
      const hB = b.height || 0;
  ...
  const checkInternal = (d: MockProduct, mode: 'standard' | 'minimal'): boolean => {
    for (const pr of transitions) {
      ...
      if (mode === 'standard') {
        if (bottomClearance < (effZDol + SAFETY) || topClearance < (clr.zGora + SAFETY)) return false;
      } else if (mode === 'minimal') {
        if (bottomClearance < (effZDolMin + SAFETY) || topClearance < (clr.zGoraMin + SAFETY)) return false;
      }
    }
    return true;
  };
...
function buildLayoutAndScore(
  ...
  minDenH: number,
  maxReqH: number,
): LayoutResult & { score: number; conf: { valid: boolean; isMinimal: boolean; errors: string[] } } {
...
function fillRingsDP(
  ...
  if (bestIdx === -1) {
    for (let i = target; i <= target + tolAbove; i++) {
      if (i < dp.length && dp[i] !== null) { bestIdx = i; break; }
    }
  }
  if (bestIdx === -1) {
    for (let i = Math.min(target, target + tolAbove); i >= Math.max(0, target - tolBelow); i--) {
      if (i < dp.length && dp[i] !== null) { bestIdx = i; break; }
    }
  }
...
function buildOTLayout(
  ...
  for (const h of holes) {
    ...
    for (let ri = 0; ri < result.length; ri++) {
      ...
      const matchesOT = holeCenter >= offsetY && holeCenter < rEnd
      ...
      if (matchesOT) {
        ...
        if (r.componentType === 'krag') {
          ...
          if (!otProd) {
            // Create dynamic OT
            otProd = { ...r, id: otId, componentType: 'krag_ot', name: r.name + ' wiercony' };
          }
          result[ri] = otProd;
        }
        break;
      ...
      for (const wh of warehouses) {
        ...
        for (const tr of stdTransitions) {
          ...
          const lifts = [
            ...
            { name: 'środek dennicy', z: Math.round(minDenH * 0.6 / 1000 * 100) / 100 },
          ...
          for (const lift of lifts) {
            ...
            cases.push({
              ...
              expectedCategory: needsTallDen && !ringHeights.length ? 'error' : 'valid',
            });
          }
        }
          ...
          for (const tr1 of stdTransitions.slice(0, 2)) {
            for (const tr2 of stdTransitions.slice(0, 2)) {
              ...
              cases.push({
                id: `C${++idx}`,
                dn,
                targetHeight: Math.max(targetH, Math.ceil((denHeights[0] + 500) / 100) * 100),
                warehouse: wh,
                rzDna: 0,
                transitions: [
        ...
        for (const tr of tightTransitions) {
          cases.push({
            id: `C${++idx}`,
            dn,
            targetHeight: Math.max(targetH, Math.ceil((denHeights[0] + 300) / 100) * 100),
            warehouse: wh,
            rzDna: 0,
            transitions: [{ productId: tr.id, rzednaWlaczenia: Math.round(denHeights[0] * 0.5 / 1000 * 100) / 100 }],
  ...
  for (const dn of dns) {
    ...
    for (const wh of warehouses) {
      cases.push({
        ...
        warehouse: wh,
        rzDna: 0,
        transitions: [{ productId: 'PRZ-160-M', rzednaWlaczenia: Math.round(denHeights[0] * 0.4 / 1000 * 100) / 100 }],
...
function validateTestCase(
  ...
  const dnResult = selectLowestDennica(allProducts, tc.dn, tc.warehouse, tc.transitions, tc.rzDna);
  ...
  // Step 2: Get rings
  const rings = allProducts.filter(p => p.componentType === 'krag' && parseInt(String(p.dn)) === tc.dn);

  // Step 3: Top closure
  const konus = allProducts.find(p => p.componentType === 'konus' && parseInt(String(p.dn)) === tc.dn);
  const topClosure = konus || allProducts.find(p => p.componentType === 'plyta_din' && parseInt(String(p.dn)) === tc.dn);
  ...
  const holes: Hole[] = tc.transitions.map(tr => {
    ...
    return { z, ruraDz: dnVal, zdD: clr.zDol, zdG: clr.zGora, zdDM: clr.zDolMin, zdGM: clr.zGoraMin };
  });
  ...
  const denCandidates = allProducts
    .filter(p => p.componentType === 'dennica' && parseInt(String(p.dn)) === tc.dn && p.height >= minDenH)
    .sort((a, b) => a.height - b.height);

  let bestResult: (LayoutResult & { score: number; conf: { valid: boolean; isMinimal: boolean; errors: string[] } }) | null = null;
  ...
  if (bestResult && bestResult.dennicaHeight > minDenH && bestResult.otCount === 0) {
    failures.push(`Higher dennica used (${bestResult.dennicaHeight}mm > ${minDenH}mm) without OT rings`);
  }
  ...
  if (hasCrossingJoint && bestResult && bestResult.otCount === 0) {
    failures.push(`Pipe crosses joint but no krag_ot used (dennica=${bestResult.dennicaHeight}mm, minDenH=${minDenH}mm)`);
  }
  ...
  if (bestResult) {
    ...
    if (hDiff > 200) {
      failures.push(`Height mismatch: got ${bestResult.totalHeight}mm, expected ~${tc.targetHeight}mm (diff=${hDiff}mm)`);
    }
  }
...
describe('Massive solver validation (500+ cases)', () => {
  ...
  afterAll(() => {
    ...
    const avgDenHeight = dennicaHeights.length > 0
      ? Math.round(dennicaHeights.reduce((a, b) => a + b, 0) / dennicaHeights.length) : 0;
    ...
    for (const r of results) {
      ...
      byDN[kdn].total++; if (r.passed) byDN[kdn].passed++;
      const tc = r.case.transitions.length === 0 ? 'none' : r.case.transitions.length === 1 ? 'single' : 'multiple';
      if (!byTransitions[tc]) byTransitions[tc] = { total: 0, passed: 0 };
      byTransitions[tc].total++; if (r.passed) byTransitions[tc].passed++;
    }
    const worstFailures = results
      .filter(r => !r.passed && r.failures.length > 0)
      .sort((a, b) => b.failures.length - a.failures.length).slice(0, 30);
    ...
    rpt.push(`  OT użyte:          ${otUsage}/${total} (${Math.round(otUsage / total * 100)}%)`);
    rpt.push('');
    rpt.push(`  ── Wyniki wg DN ──`);
    for (const [dn, s] of Object.entries(byDN)) {
      const pct = Math.round(s.passed / s.total * 100);
      const bar = '█'.repeat(Math.round(pct / 5)) + '░'.repeat(20 - Math.round(pct / 5));
      rpt.push(`  DN${String(dn).padStart(4)}: ${String(s.passed).padStart(3)}/${String(s.total).padStart(3)} (${String(pct).padStart(2)}%) ${bar}`);
    }
    rpt.push('');
    ...
    for (const [tc, s] of Object.entries(byTransitions)) {
      const pct = Math.round(s.passed / s.total * 100);
      const bar = '█'.repeat(Math.round(pct / 5)) + '░'.repeat(20 - Math.round(pct / 5));
      rpt.push(`  ${String(tc).padStart(8)}: ${String(s.passed).padStart(3)}/${String(s.total).padStart(3)} (${String(pct).padStart(2)}%) ${bar}`);
    }
    rpt.push('');
    ...
    else if (passed >= total * 0.95) rpt.push(`  WARN: ${total - passed} cases with warnings (acceptable)`);
