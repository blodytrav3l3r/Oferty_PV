...
## 50. Przejścia na łączeniach kręgów (auto-dobór: OT po środku + bypass krag_ot + cichy Excel)

**Problem**: (1) OT przypisywany po środku otworu (`center ∈ [start,end)`), a walidacja jointów po strefie z zapasami — joint w korpusie poza środkiem przechodził. (2) Bypass `hasOTAbove && jointInBody` w `checkConflicts` (`solverAutoSelect.js`) omijał walidację, gdy segment wyżej to `krag_ot`. (3) KROK3 resolve'ował produkt przejścia tylko z `availProducts` (niedostępny → phantom DN160/0 zapasów), a OT globalnie — rozjazd geometrii. (4) `excelOnPrzejscieChange` markował MANUAL przed wszystkim, więc auto-solve nigdy nie startował i stary stos zostawał przy nowej geometrii.
**Objaw**: Przejścia na łączeniach kręgów (szczególnie po edycji w Excelu); solver zwracał pozorne OT dla otworów przecinających joint.
**Fix** (plan `docs/plans/2026-09-11-auto-dobor-przejscia-na-laczeniach.md`): SSoT `transitionZones.js` (`getTransitionDn/getTransitionBody/getTransitionZone/segmentContainsBody/jointInZone`); OT = containment korpusu w 3 miejscach (`buildCandidateLayouts`, `checkSegmentHasHole`, `applyDrilledRings`); kasacja bypassu; KROK3 resolve globalnie (jak OT i ścieżka redukcyjna); joint w strefie minimalnej → twardy ERROR `Kolizja otworu Z=… (strefa minimalna)` propagowany z odrzuconych kandydatur; Excel: zmiana `rzednaWlaczenia`/`productId` w AUTO → pełny re-solve, MANUAL/angle bez solvera. Bez zmian DP, scoringu i `isMinimal`. Testy: `transitionZoneJoints.test.ts` (macierz A-E + kontrakt P2); goldeny `solverEquivalence` dla 5 przypadków z przejściami zmieniono świadomie (fixture GRP DN1000 — fizycznie niewykonalne, stąd ERROR zamiast pozornego OT).

## 51. Ghost-draft oferty studni (komparator widział pola efemeryczne load)

**Problem**: Pipeline load (`normalizeOfferData`/`ensureElemIds` z losowym `_elemId`, `recalcGaskets`, `syncKineta`) doklejał do live pola, których SAVED nie ma (`_elemId` w pozycjach, `configStatus`/`configErrors`/`wellHeight` w studni) — oferta nie miała projekcji live→SAVED (zamówienie ma `toOrderWellsDTO`), więc flush przy unload pisał ghosta, a martwy klucz nigdy sam nie znikał (gate robił tylko `return false`). Dodatkowo cache `offersStudnie` po `ensureOfferDetail` zostawał slim/stale, więc recovery decydowało na niepełnych danych.
**Objaw**: „Znaleziono niezapisany draft" po każdym wejściu mimo zapisanych zmian.
**Fix**: `_draftComparablePayload` stripuje efemeryczne (well-level + item-level, payload draftu nietknięty — restore w całości), `transportMode undefined` = `'full'`; self-heal `_draftRemoveIfDead` kasuje klucz gdy draft≡SAVED (flush i entry), treść obca nietknięta; write-back detalu do `offersStudnie` w `loadSavedOfferStudnie`; `logger.warn` z różniącymi sekcjami przed modalem. Test: `draftStudnieOfferRoundtrip.test.ts` (5 przypadków).

## 52. Ghost-draft zamówienia studni po dodaniu do oferty (brak wizard/pól w kreacji)

**Problem**: `createOrderFromOffer` buduje dokument bez `wizard` i często bez `validity`/`paymentTerms`, a live zawsze zbiera pełny wizard z leftover tiles (`getWizardGlobalParams`) + fallbacki UI (`validity` → `'7 dni'`, `transport-km` → `?? 100`). Żaden save zamówienia wizarda nie dopisuje (PATCH wells-only), więc live≠SAVED na zawsze → ghost przy każdym fluszu, modal przy każdym wejściu (tile-state losowy między sesjami).
**Objaw**: „Znaleziono niezapisany draft" na zamówieniu zaraz po jego dodaniu do oferty.
**Fix**: `_draftComparablePayload` — zamówienia bez kluczy wizarda (obie strony; tiles w trybie order to szum, step wymuszony 5); brak klucza pola = fallback UI (`transportKm` 100, `transportRate` 10, `validity` '7 dni', `paymentTerms` default, reszta stringów `''`); `date`/`number` drop gdy brak po którejkolwiek stronie (dryf dzienny). Bez zmian UI i ścieżek SAVED. Test: `draftStudnieOrderCreate.test.ts` (3 przypadki).

## 53. Hover przejścia z podglądu martwy na danych legacy (brak pr.id)

**Problem**: Ścieżka hover SVG → Excel/kafelki wymaga stabilnego `pr.id` (`data-prz-id`, `resolvePrzTarget` w `diagramRenderer.js`). Zamówienia zapisane przed `ensurePrzejsciaIds` (np. SA/ZS/000050/2026 — wszystkie `pr.id === undefined`) renderowały `<g data-prz-id="">`, a `resolvePrzTarget('')` zwraca null — hover nie podświetlał NICZEGO. Dodatkowo klik w kształt nie istniał w ogóle (brak `onclick`), a modal Excela resetuje kontekst (`_excelActiveTab = DN_TABS[0]`, `currentWellIndex = -1`), więc wiersz studni bywał poza DOM.
**Objaw**: Hover przejścia w podglądzie studni nie podświetla komórek Excela (0/4 zamiast 4/4); klik bez efektu.
**Fix**: (1) Backfill `ensurePrzejsciaIds(well.przejscia)` na wejściu `renderWellDiagram` — jeden guard naprawia ~38 callerów (konfigurator, zlecenia, Excel). (2) Trwała selekcja po kliku: `svgPrzPointerClick` (toggle, klasy `.excel-tr-selected` / `.prz-tile--svg-selected`, scroll do komórki, auto-switch taba DN właściciela gdy wiersz niewidoczny; hover selekcji nie rusza — osobne klasy; re-apply po re-renderze). (3) Hover kafelka zawsze (też bez drag / bez przypisania — po `tileId`, nie `globalIndex`). Testy: `svgPrzHighlight.test.ts` (15 przypadków), `tests/playwright/excelPrzHighlight.cjs` (żywe dane SA/ZS/000050/2026 s1: 5/5 kształtów, hover 4/4, klik 4/4 + tab 1000, toggle off).

## 54. Wyścig fokusu quick-edit: Chromium focus-fixup + async populate vs activeElement

**Problem**: Przełączanie pól quick-edit Listy przejść w Zleceniach Produkcyjnych wymagało 2 klików. Kaskada przyczyn: (1) `saveQuickEdit` planował pełny refresh (lista + `populateZleceniaForm` z async fetch) 100 ms po blurze — lądował po utworzeniu nowego inputa i go niszczył; (2) `rebuildInput` liczył kontener z `element.closest()` na odłączonym nodzie po przerwie async (fallback do ukrytego konfiguratora); (3) guard „nie kradnij fokusu" brał odłączony `activeElement` (przeglądarka trzyma go do async blur) za nowsze pole; (4) `preventDefault()` na mousedown + sync `focus()` walczył z focus-fixupem Chromium (focusout 1–2 ms po focusin, click w ogóle nie docierał); (5) ciężkie rendery (diagram/summary/config) w tasku clicka psuły hit-test domyślnej akcji fokusu; (6) deferred heavy-refresh wołał `updateSummary → renderWellsList → refreshAllWellErrors → renderWellConfigErrors`, który bezwarunkowo robił pełny refresh modala (`populate` niszczył input); (7) stempel `qeApplied` stawiany na `document.activeElement` mógł oznaczyć OBCE (nowe) pole i zgubić jego późniejszy zapis (data-loss).
**Objaw**: Klik w inne pole najpierw gasił aktywne, drugie kliknięcie otwierało nowe; fokus ginął 1–2 ms po focusin (E2E focus-log).
**Warunki**: modal PZ (`#zl-przejscia-list` z `filterElementIndex`); w konfiguratorze ten sam tor działał (lekki rerender).
**Fix** (`wellTransitions.js`, `wellActions.js`, `transitionRenderer.js`, `solverValidation.js`): tor naturalny mousedown→blur→click (handler mousedown usunięty); zapis przy fokusie w polu QE synchroniczny i cichy (`isQeInputFocused` + `runQeSilent`: brak refresha listy/modala, ciężkie rendery odroczone `scheduleQeHeavyRefresh` poza task clicka z koalescencją); kontener zapamiętany na wejściu (`entryContainerId`); guard fokusu ignoruje odłączony node (`isConnected !== false`); `renderWellConfigErrors` pomija refresh modala przy fokusie QE (banner sync na wyjściu z edycji); stempel `qeApplied` usunięty (cicha ścieżka + idempotentny zapis go zastępują); allowlista pól (`QE_FIELD_ALLOWLIST`) + `qeAttrEscape` w selektorze; recheck locków na starcie `applyChanges` (F2); `flushQePendingState()` na wejściu `closeZleceniaModal` (F3); tryb `light` w `refreshZleceniaModalIfActive`.
**Ograniczenia**: pełny obraz listy (re-sort/filtr po zmianie rzędnej) dopiero przy blur poza pola; banner błędów PZ sync na wyjściu z edycji.
