...
> **Typ bazy a komenda synchronizacji:** Bazy tworzone przez `prisma db push` nie mają tabeli
> `_prisma_migrations` — na nich `prisma migrate deploy` zawodzi. `install.bat`/`install.sh`
> automatycznie wybierają właściwą komendę (migrate deploy z fallbackiem `db push`).
...
| `sqlite: no such table`            | Nie zsynchronizowano schematu        | `start.bat` sam wykryje braki (check-db.js) i uruchomi `db push`; ręcznie: `npx prisma db push --skip-generate --accept-data-loss`                                         |
| `[check-db] BRAK INDEKSOW`         | Brak indeksów dedup telemetrii       | `start.bat`/`ensure-db` uruchomi `db push`; przy starcie serwera indeksy `idx_logs_well` / `idx_logs_source_well` tworzone są też automatycznie (auto-heal w `src/app.ts`) |
