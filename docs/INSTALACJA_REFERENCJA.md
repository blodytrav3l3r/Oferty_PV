...
> **Typ bazy a komenda synchronizacji:** Bazy legacy tworzone przez `prisma db push` nie mają tabeli
> `_prisma_migrations` — na nich `prisma migrate deploy` zawodzi. `install.bat`/`install.sh`
> uruchamiają `migrate deploy` domyślnie; fallback `db push` jest automatycznie stosowany
> tylko dla baz legacy (sprawdza `scripts/check-legacy-db.js`).
> Sprawdzenie typu bazy: `npx prisma migrate status` — jeśli pokazuje migracje jako
> niezastosowane mimo działającej aplikacji, baza jest typu `db push` (używaj `db push`).
...
| `sqlite: no such table`            | Nie zsynchronizowano schematu        | `start.bat` sam wykryje braki (check-db.js) i uruchomi `migrate deploy`; ręcznie: `npx prisma migrate deploy` (legacy: `npx prisma db push --skip-generate --accept-data-loss`)   |
| `[check-db] BRAK INDEKSOW`         | Brak indeksów dedup telemetrii       | `start.bat`/`ensure-db` uruchomi `migrate deploy`; przy starcie serwera indeksy `idx_logs_well` / `idx_logs_source_well` tworzone są też automatycznie (auto-heal w `src/app.ts`) |
| Brak wyników w wyszukiwarce ofert  | Brak tabeli FTS5 `offers_search_fts` | Tworzona automatycznie przy starcie serwera (`src/utils/fts5Sync.ts`, z backfillem ofert) — zrestartuj aplikację                                                                  |
...
| Synchronizacja schematu (baza legacy bez `_prisma_migrations`) | `npx prisma db push --skip-generate --accept-data-loss`       | Ręcznie (tylko legacy)            |
