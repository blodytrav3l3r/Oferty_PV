...
> zrealizowany później w 19778a5/0542b2b).
...
| **H: pille małe**         | Zostają jako pille; radius 20 px → `var(--radius-lg)` (badge-role:729, launch-btn-label:390, ~~dash-role-badge:1889~~, recycled-badge:476, admin-edit-badge:954)                                                                            | SPA ma pille (U9) — kształt jest spójny; tokenizacja tylko nadaje nazwę wartości. Opcjonalne (krok niski priorytet). Uwaga: `.dash-role-badge` został usunięty w commicie 0542b2b (unifikacja nagłówka na wspólne klasy `.header-*`) — ta pozycja jest nieaktualna.                                                                                                                                                            |
...
- **Plik**: `public/css/index.css`
- **Zmiany**: `border-radius: 20px` → `var(--radius-lg)` w: `.badge-role` (729), `.launch-btn-label` (390), ~~`.dash-role-badge` (1889)~~ — klasa usunięta w commicie 0542b2b (unifikacja nagłówka na `.header-*`), `.recycled-badge` (476), `.admin-edit-badge` (954).
