...
## Sekcja 11: ⚪ TECHNICAL DEBT (Backlog) — [STATUS: CZĘŚCIOWO]

**Cel:** Dług techniczny do zaplanowania w przyszłych sprintach.

**Czas:** ~8-16h łącznie

### 11.1 Podział `solverAutoSelect.js` (1205 linii) — [STATUS: CZĘŚCIOWO]

- Usunięto 369 linii martwego kodu z `solverHeuristics.js` (funkcje na `window` nigdy nie wywoływane)
- Pełny split (solverCore/solverHeuristics/solverUI) odłożony (YAGNI — closure jest lepszy niż global scope)

### 11.2 Wydzielenie `pricingCalculator.js` z `offerSave.js` — [STATUS: DONE ✅]

- `pricingCalculator.js` już istnieje z `calculateOfferPricing()`
- `offerSave.js` woła `calculateOfferPricing()` od dawna

### 11.3 Cleanup event listenerów (wycieki pamięci) — [STATUS: DONE ✅]

- `excelModal.js` — wyciągnięto anonimowe handler do nazwanych `_excelOnFocusInRow`/`_excelOnClickCell`, dodano removeEventListener
- `offerSvgDrag.js` — zapisano handler `_onSyncStatusChanged` + cleanup `window._cleanupSvgDrag`
- `mlDualRanking.js` — bez zmian (setInterval poprawnie czyszczone)
- `ui.js` — bez zmian (wzorce poprawnie sparowane)

### 11.4 CSP: `'unsafe-inline'` → nonce-based — [STATUS: DEFERRED]

**Raport:** 26 plików HTML z inline onclick (~200 handlerów). `innerHTML` wszechobecny — blokuje strict CSP.
**Rekomendacja:** Fazy 1-4 (nonce → onclick migration → textContent → strict CSP). Szacowany czas: 6-8h.
**Decyzja:** Odłożone do momentu podjęcia decyzji architektonicznej o refaktorze UI.

### 11.5 `Record<string, unknown>` → konkretne interfejsy — [STATUS: DONE ✅]

- `searchUtils.ts:mapOfferRow` — dodano interfejs `SearchOfferRowMapped` z konkretnymi polami
- Pozostałe 100+ wystąpień w `src/` pozostawione (Express query params, Prisma queryRaw — właściwe typy dla tych kontekstów)
...
| ------------------ | ------------- | :------------: | :-------: |
| 1. Krytyczne XSS   | 🔴 Dzień 1    |     30 min     |  DONE ✅  |
| 2. SQL Injection   | 🔴 Dzień 1    |     15 min     |  DONE ✅  |
| 3. Wysokie XSS     | 🟡 Dzień 2    |     30 min     |  DONE ✅  |
| 4. Bezpieczeństwo  | 🟡 Dzień 2    |     15 min     |  DONE ✅  |
| 5. Martwy kod      | 🟡 Dzień 3    |     30 min     |  DONE ✅  |
| 6. Literówki       | 🟢 Dzień 3    |     15 min     |  DONE ✅  |
| 7. Bugi logiczne   | 🟢 Dzień 4    |     20 min     |  DONE ✅  |
| 8. CSS/HTML        | 🟢 Dzień 4    |     30 min     |  DONE ✅  |
| 9. Refaktor typów  | 🔵 Sprint 2   |       2h       |  DONE ✅  |
| 10. Wspólny CRUD   | 🔵 Sprint 2-3 |       4h       |  DONE ✅  |
| 11. Technical Debt | ⚪ Backlog    |     8-16h      | CZĘŚCIOWO |
