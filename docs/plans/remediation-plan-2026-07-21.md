...
### 11.4 CSP: `'unsafe-inline'` → nonce-based — [STATUS: ANALYZED]

**Raport:** 26 plików HTML z inline onclick (~200 handlerów). 4 pliki partials z największą liczbą. `innerHTML` wszechobecny — blokuje strict CSP.
**Rekomendacja:** Fazy 1-4 (nonce → onclick migration → textContent → strict CSP). Szacowany czas: 6-8h.
**Blocker:** `innerHTML` w dynamicznym UI wymaga migracji na `textContent` lub Trusted Types.
